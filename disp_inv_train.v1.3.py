...
"""
Conditional Rectified Flow inversion training v1.2:
...
The CRF target is a compact control-point representation of [Vp, Vs, rho].
Control points are linearly interpolated back to the full depth grid before
the reconstruction/smoothness losses are evaluated. This reduces high-frequency
point-wise noise in inversion samples.
"""
...
def select_control_indices(depth_km: torch.Tensor) -> torch.Tensor:
    """
    Piecewise depth control points:
      0-5 km    : 0.5 km
      5-10 km   : 1 km
      10-50 km  : 5 km
      50-150 km : 20 km
    ...
    cp_depths = np.unique(np.asarray([zmin] + cp_depths + [zmax], dtype=float))
    cp_idx = [int(np.argmin(np.abs(depth_np - z))) for z in cp_depths]
    cp_idx = np.unique(np.asarray(cp_idx, dtype=int))
    return torch.as_tensor(cp_idx, dtype=torch.long, device=depth_km.device)


def build_interp_indices(depth_full: torch.Tensor, depth_cp: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if depth_cp.numel() == 1:
        zeros = torch.zeros_like(depth_full, dtype=torch.long)
        weight = torch.zeros_like(depth_full)
        return zeros, zeros, weight

    right = torch.searchsorted(depth_cp, depth_full).clamp(1, depth_cp.numel() - 1)
    left = right - 1

    z0 = depth_cp[left]
    z1 = depth_cp[right]
    weight = (depth_full - z0) / (z1 - z0).clamp_min(1e-12)

    top = depth_full <= depth_cp[0]
    bottom = depth_full >= depth_cp[-1]
    left = torch.where(top, torch.zeros_like(left), left)
    right = torch.where(top, torch.zeros_like(right), right)
    weight = torch.where(top, torch.zeros_like(weight), weight)
    last = torch.full_like(left, depth_cp.numel() - 1)
    left = torch.where(bottom, last, left)
    right = torch.where(bottom, last, right)
    weight = torch.where(bottom, torch.zeros_like(weight), weight)
    return left.long(), right.long(), weight
...
class Disp2StructCRF(nn.Module):
    """
    Conditional Rectified Flow from masked dispersion curves to control-point
    velocity profiles, then linear interpolation to the full depth grid.
    """
    ...
    def __init__(
        ...
        control_indices: Optional[torch.Tensor] = None,
        ...
        depth_grid = torch.as_tensor(depth_grid, dtype=torch.float32).reshape(H)
        if control_indices is None:
            control_indices = select_control_indices(depth_grid)
        control_indices = torch.as_tensor(control_indices, dtype=torch.long).reshape(-1)
        control_indices = torch.unique(control_indices.clamp(0, H - 1), sorted=True)
        self.Nc = int(control_indices.numel())
        self.output_dim = profile_channels * self.Nc
        control_depth_grid = depth_grid.index_select(0, control_indices)
        interp_left, interp_right, interp_weight = build_interp_indices(depth_grid, control_depth_grid)
    ...
    def profile_to_full_z(self, profile: torch.Tensor) -> torch.Tensor:
    ...
    def profile_to_control(self, profile: torch.Tensor) -> torch.Tensor:
        return profile.index_select(dim=-1, index=self.control_indices)

    def control_to_z(self, control_profile: torch.Tensor) -> torch.Tensor:
        ref = self.reference_profile.index_select(dim=-1, index=self.control_indices)
        scale = self.profile_scale.index_select(dim=-1, index=self.control_indices)
        return (control_profile - ref.view(1, self.profile_channels, self.Nc)) / scale.view(
            1, self.profile_channels, self.Nc
        )

    def z_to_control(self, z: torch.Tensor) -> torch.Tensor:
        ref = self.reference_profile.index_select(dim=-1, index=self.control_indices)
        scale = self.profile_scale.index_select(dim=-1, index=self.control_indices)
        if z.ndim == 3:
            return z * scale.view(1, self.profile_channels, self.Nc) + ref.view(1, self.profile_channels, self.Nc)
        if z.ndim == 4:
            return z * scale.view(1, 1, self.profile_channels, self.Nc) + ref.view(
                1, 1, self.profile_channels, self.Nc
            )
        raise ValueError(f"Expected z [B,C,Nc] or [B,S,C,Nc], got {tuple(z.shape)}")

    def profile_to_z(self, profile: torch.Tensor) -> torch.Tensor:
        return self.control_to_z(self.profile_to_control(profile))

    def control_to_profile(self, control_profile: torch.Tensor) -> torch.Tensor:
        if control_profile.ndim == 3:
            left = self.interp_left_indices.view(1, 1, self.H).expand(
                control_profile.size(0), self.profile_channels, self.H
            )
            right = self.interp_right_indices.view(1, 1, self.H).expand_as(left)
            weight = self.interp_weight.to(dtype=control_profile.dtype).view(1, 1, self.H)
            left_val = control_profile.gather(dim=-1, index=left)
            right_val = control_profile.gather(dim=-1, index=right)
            return (1.0 - weight) * left_val + weight * right_val
        if control_profile.ndim == 4:
            left = self.interp_left_indices.view(1, 1, 1, self.H).expand(
                control_profile.size(0), control_profile.size(1), self.profile_channels, self.H
            )
            right = self.interp_right_indices.view(1, 1, 1, self.H).expand_as(left)
            weight = self.interp_weight.to(dtype=control_profile.dtype).view(1, 1, 1, self.H)
            left_val = control_profile.gather(dim=-1, index=left)
            right_val = control_profile.gather(dim=-1, index=right)
            return (1.0 - weight) * left_val + weight * right_val
        raise ValueError(f"Expected control profile [B,C,Nc] or [B,S,C,Nc], got {tuple(control_profile.shape)}")

    def z_to_profile(self, z: torch.Tensor) -> torch.Tensor:
        return self.control_to_profile(self.z_to_control(z))
    ...
    def sample(
        ...
        z_samples = z.view(batch_size, num_samples, self.profile_channels, self.Nc)
        control_samples = self.z_to_control(z_samples)
        profile_samples = self.control_to_profile(control_samples)
...
def compute_total_loss(
    ...
    target_full_z = model.profile_to_full_z(target_profile)
    ...
    pred_profile_1 = model.z_to_profile(pred_z_1)
    pred_full_z_1 = model.profile_to_full_z(pred_profile_1)
    rec_loss = smooth_l1_mean(pred_full_z_1, target_full_z, beta=flow_beta)
    slope = slope_loss(pred_full_z_1, target_full_z)
    curvature = curvature_loss(pred_full_z_1, target_full_z)
    ...
    return {
        ...
        "pred_profile_train": pred_profile_1.detach(),
    }
...
def parse_period_windows(spec: str) -> Tuple[Tuple[float, float], ...]:
    """Parse strings such as ``2-40,10-30`` into period-window tuples."""
    windows = []
    for part in str(spec).split(","):
        ...
        if not part:
            continue
        if "-" not in part:
            raise ValueError(f"Period-window entry must be like '10-30', got {part!r}")
        lo, hi = part.split("-", 1)
        lo_f = float(lo)
        hi_f = float(hi)
        if hi_f <= lo_f:
            raise ValueError(f"Invalid period window {part!r}: upper bound must exceed lower bound")
        windows.append((lo_f, hi_f))
    return tuple(windows)


def _apply_single_period_window(
    periods: torch.Tensor,
    wave_mask: torch.Tensor,
    start_s: torch.Tensor,
    end_s: torch.Tensor,
    max_internal_holes: int,
    hole_point_prob: float,
) -> torch.Tensor:
    """Apply one continuous period window plus random missing points to one example."""
    keep = (periods >= start_s) & (periods <= end_s)
    if keep.sum() == 0:
        nearest = torch.argmin(torch.abs(periods - 0.5 * (start_s + end_s)))
        keep[nearest] = True

    out = wave_mask * keep.to(dtype=wave_mask.dtype).view(1, -1)

    valid_idx = torch.nonzero(keep, as_tuple=False).flatten()
    if valid_idx.numel() > 2 and max_internal_holes > 0:
        n_holes = int(torch.randint(0, max_internal_holes + 1, (1,), device=periods.device).item())
        if n_holes > 0:
            choose = torch.randint(0, valid_idx.numel(), (n_holes,), device=periods.device)
            out[:, valid_idx[choose]] = 0.0

    if hole_point_prob > 0.0:
        point_keep = (torch.rand(periods.shape, device=periods.device) >= float(hole_point_prob)).to(out.dtype)
        out = out * point_keep.view(1, -1)

    if out.sum() == 0:
        out[:, valid_idx[valid_idx.numel() // 2]] = wave_mask[:, valid_idx[valid_idx.numel() // 2]]
    return out


def augment_dispersion_period_mask(
    disp: torch.Tensor,
    mask: torch.Tensor,
    *,
    enabled: bool = True,
    preset_windows: Tuple[Tuple[float, float], ...] = ((2.0, 40.0), (10.0, 30.0)),
    preset_prob: float = 0.65,
    random_min_width_s: float = 15.0,
    random_max_width_s: float = 45.0,
    max_internal_holes: int = 6,
    hole_point_prob: float = 0.04,
) -> torch.Tensor:
    """Randomly restrict valid dispersion observations to intermittent period windows.

    The period samples remain on the original grid, but Rayleigh/Love masks are
    multiplied by a random continuous window and optional internal missing
    points. This emulates field dispersion picks that may only exist on, for
    example, 10--30 s or 2--40 s, while keeping the network architecture
    unchanged.
    """
    if not enabled:
        return mask
    if mask.ndim != 3 or mask.size(1) < 3:
        raise ValueError(f"Expected mask [B,3,T], got {tuple(mask.shape)}")

    out = mask.clone()
    periods = disp[:, 0, :]
    bsz = disp.size(0)
    presets = tuple((float(lo), float(hi)) for lo, hi in preset_windows)
    use_preset = presets and (torch.rand(bsz, device=disp.device) < float(preset_prob))

    for i in range(bsz):
        p = periods[i]
        pmin = p.min()
        pmax = p.max()
        if bool(use_preset[i]):
            j = int(torch.randint(0, len(presets), (1,), device=disp.device).item())
            start_s = torch.tensor(presets[j][0], device=disp.device, dtype=p.dtype).clamp(pmin, pmax)
            end_s = torch.tensor(presets[j][1], device=disp.device, dtype=p.dtype).clamp(pmin, pmax)
            if end_s <= start_s:
                start_s, end_s = pmin, pmax
        else:
            max_width = min(float(random_max_width_s), float((pmax - pmin).detach().cpu()))
            min_width = min(float(random_min_width_s), max_width)
            if max_width <= 0.0:
                start_s, end_s = pmin, pmax
            else:
                width = min_width + torch.rand((), device=disp.device, dtype=p.dtype) * max(max_width - min_width, 1e-6)
                start_s = pmin + torch.rand((), device=disp.device, dtype=p.dtype) * (pmax - pmin - width).clamp_min(0.0)
                end_s = start_s + width

        out[i, 1:3, :] = _apply_single_period_window(
            p,
            out[i, 1:3, :],
            start_s,
            end_s,
            max_internal_holes=max_internal_holes,
            hole_point_prob=hole_point_prob,
        )
        out[i, 0, :] = (out[i, 1:3, :].sum(dim=0) > 0).to(out.dtype)
    return out
...
def estimate_training_stats(
    ...
    mask_augment_kwargs: Optional[Dict] = None,
) -> Dict[str, torch.Tensor]:
    profile_sum = None
    profile_sq_sum = None
    profile_count = 0
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if mask_augment_kwargs:
            mask_batch = augment_dispersion_period_mask(disp_batch, mask_batch, **mask_augment_kwargs)
...
def append_csv_row(path: str, row: Dict) -> None:
    ...
    exists_file = os.path.exists(path) and os.path.getsize(path) > 0
    if exists_file:
...
def runtime_metadata(device: torch.device, cfg: "TrainConfig") -> Dict:
    meta = {
    ...
    if torch.cuda.is_available():
        ...
        meta["cuda_devices"] = [torch.cuda.get_device_name(i) for i in range(torch.cuda.device_count())]
    return meta
...
class TrainConfig:
    ...
    save_dir: str = "ckpt/disp2struct_crf.v1.3_cp"
    fig_dir: str = "tfig_inv_v1.3_cp"
    resume: bool = True
    init_checkpoint: Optional[str] = None

    mask_augment: bool = True
    mask_augment_val: bool = True
    mask_window_presets: Tuple[Tuple[float, float], ...] = ((2.0, 40.0), (10.0, 30.0), (5.0, 35.0), (15.0, 45.0))
    mask_window_preset_prob: float = 0.65
    mask_random_min_width_s: float = 15.0
    mask_random_max_width_s: float = 45.0
    mask_internal_holes_max: int = 6
    mask_hole_point_prob: float = 0.04
    ...
    epoch_metrics_filename: str = "epoch_metrics.csv"
    epoch_metrics_jsonl: str = "epoch_metrics.jsonl"
    ...
    torch.save(
        {
            ...
            "config": asdict(cfg),
            "model_name": "Disp2StructControlPointCRFV13",
            ...
            "control_indices": model.control_indices.detach().cpu(),
            "control_depth_grid": model.control_depth_grid.detach().cpu(),
            "interp_left_indices": model.interp_left_indices.detach().cpu(),
            "interp_right_indices": model.interp_right_indices.detach().cpu(),
            "interp_weight": model.interp_weight.detach().cpu(),
...
def load_initial_network_weights(model: Disp2StructCRF, ckpt_path: str, device: torch.device) -> None:
    """Warm-start trainable network weights while keeping current normalization buffers."""
    ...
    print(
        f"Warm-started {len(compatible)} tensors from {ckpt_path} "
...
def train_disp2struct_crf(
    ...
    epoch_csv_path = os.path.join(cfg.save_dir, cfg.epoch_metrics_filename)
    epoch_jsonl_path = os.path.join(cfg.save_dir, cfg.epoch_metrics_jsonl)

    set_seed(cfg.seed)
    device = torch.device(cfg.device)
    print("Device:", device)
    write_json(os.path.join(cfg.save_dir, "config_resolved.json"), asdict(cfg))
    write_json(os.path.join(cfg.save_dir, "runtime_metadata.json"), runtime_metadata(device, cfg))
    train_mask_augment_kwargs = {
    ...
    val_mask_augment_kwargs = dict(train_mask_augment_kwargs)
    val_mask_augment_kwargs["enabled"] = cfg.mask_augment and cfg.mask_augment_val
    ...
    stats = estimate_training_stats(
        ...
        mask_augment_kwargs=train_mask_augment_kwargs,
    )
    torch.save(stats, os.path.join(cfg.save_dir, "normalization_stats.pt"))
    write_json(
        os.path.join(cfg.save_dir, "normalization_stats_summary.json"),
        {
            "H": int(stats["H"].item()),
            "T": int(stats["T"].item()),
            "period_minmax": stats["period_minmax"],
            "disp_mean": stats["disp_mean"],
            "disp_scale": stats["disp_scale"],
            "stats_batches": int(stats["stats_batches"].item()),
        },
    )
    H = int(stats["H"].item())
    T = int(stats["T"].item())
    ...
    if cfg.init_checkpoint and not cfg.resume:
        load_initial_network_weights(model, cfg.init_checkpoint, device)
    ...
    print("========== Disp2Struct Control-Point CRF v1.3 Training Start ==========")
    print(f"H/T/Nc        : {H}/{T}/{model.Nc}")
    print(f"period range  : {period_minmax[0]:.2f} - {period_minmax[1]:.2f} s")
    print(f"mask augment  : enabled={cfg.mask_augment}, val={cfg.mask_augment_val}")
    print(f"mask presets  : {cfg.mask_window_presets}, preset_prob={cfg.mask_window_preset_prob:.2f}")
    print(
        f"random window : {cfg.mask_random_min_width_s:.1f}-{cfg.mask_random_max_width_s:.1f} s, "
        f"holes<= {cfg.mask_internal_holes_max}, point_drop={cfg.mask_hole_point_prob:.3f}"
    )
    ...
    write_log_line(
        log_path,
        (
            f"========== Disp2Struct Control-Point CRF v1.3 Training Start | start_epoch={start_epoch} "
            f"global_step={global_step} H={H} T={T} Nc={model.Nc} "
            f"mask_augment={cfg.mask_augment} presets={cfg.mask_window_presets} "
            f"random_width={cfg.mask_random_min_width_s}-{cfg.mask_random_max_width_s} "
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if val_loader is not None and epoch % cfg.val_every == 0:
            val_stats, _ = run_one_epoch(
                ...
                plot_num_samples=cfg.plot_num_samples,
                fig_dir=None,
                mask_augment_kwargs=val_mask_augment_kwargs,
            )
        ...
        if val_stats is not None:
            msg += (
                ...
                f"rec={val_stats['rec_loss']:.5f}, slope={val_stats['slope_loss']:.5f}, "
                f"curv={val_stats['curvature_loss']:.5f}, "
        ...
        row = {
            ...
            "train_slope_loss": train_stats["slope_loss"],
            "train_curvature_loss": train_stats["curvature_loss"],
            "train_mae": train_stats["mae"],
            "train_rmse": train_stats["rmse"],
            "train_vp_mae": train_stats["vp_mae"],
            "train_vs_mae": train_stats["vs_mae"],
            "train_rho_mae": train_stats["rho_mae"],
        }
        if val_stats is not None:
            row.update(
                {
                    ...
                    "val_rec_loss": val_stats["rec_loss"],
                    "val_slope_loss": val_stats["slope_loss"],
                    "val_curvature_loss": val_stats["curvature_loss"],
                    "val_mae": val_stats["mae"],
                    "val_rmse": val_stats["rmse"],
                    "val_vp_mae": val_stats["vp_mae"],
                    "val_vs_mae": val_stats["vs_mae"],
                    "val_rho_mae": val_stats["rho_mae"],
                    "val_sample_mae": val_stats["sample_mae"],
                    "val_sample_rmse": val_stats["sample_rmse"],
                    "val_sample_vp_mae": val_stats["sample_vp_mae"],
                    "val_sample_vs_mae": val_stats["sample_vs_mae"],
                    "val_sample_rho_mae": val_stats["sample_rho_mae"],
                }
            )
        ...
        row["best_val_loss_before_epoch"] = best_val_loss
        row["is_best"] = bool(score < best_val_loss)
        append_csv_row(epoch_csv_path, row)
        append_jsonl(epoch_jsonl_path, row)
        if score < best_val_loss:
            ...
            write_json(
                os.path.join(cfg.save_dir, "best_selection.json"),
                {
                    "best_checkpoint": best_path,
                    "epoch": epoch,
                    "global_step": global_step,
                    "selection_metric": "val_loss" if val_stats is not None else "train_loss",
                    "best_val_loss": best_val_loss,
                    "row": row,
                },
            )

    print("========== Disp2Struct Control-Point CRF v1.3 Training Done ==========")
    print(f"Best score: {best_val_loss:.6f}")
    write_json(
        os.path.join(cfg.save_dir, "training_complete.json"),
        {
            ...
            "best_checkpoint": os.path.join(cfg.save_dir, "best.pt"),
        },
    )
    return model
