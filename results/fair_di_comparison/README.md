...
python scripts/eval_fair_sampling_sensitivity.py \
  --ckpt ckpt/fair_di_weak_full_seed642026/best.pt \
  --out-dir results/fair_di_comparison/production/sampling_sensitivity \
  --n-test 128 \
  --posterior-samples 16 32 64 128 \
  --euler-steps 12 24 48 \
  --regimes in-prior boundary \
  --batch-size 16
...
`field_posterior_predictive.json`. Sampling sensitivity writes
`sampling_sensitivity.csv/json`.
...
## Current production status and pre-submission archive tasks
...
Archive-release tasks that are outside the manuscript text are tracked in
`pre_submission_blockers.md` and `archive_manifest_GJI.md`.
